import numpy  as np
import ez_numerics_blas as ez_numerics #okay, this is too coy, but whatever
...
def plot_tau_vs_rex_no_mu_inf(alpha):
    ...
    Res = np.logspace(2, 10, num=54)
    Des = np.logspace(2, 8, num=3)/10
    
    # Now, we simply get Newtonian shear plot
    kappa_n = 0.33205733621519
    tau_n = kappa_n * Res**-.5
...
def compute_suite_of_profiles_hakuna_matata_transposed(pd, eta_max=10):
    ...
    # Now, let's get the Power law solution 
    etap, y = ez_numerics.compute_power_law_ivp_soln(pd.alpha, ez_numerics.make_pl_default_unstable_solution_data(pd.alpha))
    uxp = y[1,:]
    eta0p = etap * ((1-pd.mu_inf)**(1/(pd.alpha+1))) / (pd.delta**((1-pd.alpha)/(2+2*pd.alpha)))
    plt.plot(uxp, eta0p, 'r-', label='Power Law')
    
    # Now we add in Newtonian0
    eta00, y = ez_numerics.compute_power_law_ivp_soln(1, ez_numerics.make_pl_default_unstable_solution_data(1))
    uxn0 = y[1,:]
