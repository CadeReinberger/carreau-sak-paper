...
def plot_alpha_03():
    ...
    plt.ylabel(r"$f_p$", fontsize=40)
    # plt.title("New Solutiuon Asymptotic For Shear-Thinning Blasius")
    plt.legend()
    plt.tight_layout()
    plt.tick_params(axis='both', labelsize=15)
    plt.savefig("new_thinning_asymptotic.eps", format='eps', bbox_inches='tight')
    plt.close()
...
def plot_alpha_14_f4():
    ...
    plt.xlabel(r"$\eta$", fontsize=40)
    plt.ylabel(r"$f_p^{(4)}$", fontsize=40)
    # plt.title("Singular Solution in Shear-Thickening Case for Power-Law Sakiadis")
    # plt.legend()
    plt.tight_layout()
    plt.tick_params(axis='both', labelsize=15)
    plt.savefig("new_thickening_deriv.eps", format='eps')
    plt.close()
